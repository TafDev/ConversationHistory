...
RSpec.describe ProjectHistory do
  describe 'validations' do
    ...
    it { is_expected.to validate_presence_of(:event_body) }
  end
  ...
  describe 'enums' do
    it { is_expected.to define_enum_for(:event_type).with_values(comment: 0, status_change: 1) }
  end

  describe 'scopes' do
    let!(:project) { create(:project) }
    let!(:comment_history) { create(:project_history, project:, event_type: 'comment') }
    let!(:status_change_history) { create(:project_history, project:, event_type: 'status_change') }
